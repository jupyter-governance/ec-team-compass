...
This page explains how grants and other funds flow through the LF structures that hold Jupyter's money to document where the money can land, how much overhead is taken out, and who controls how it is spent.
...
Example: Money received into LF Charities and then transferred to the Jupyter Foundation is charged the G&A fee when it lands in the Jupyter Foundation. If the Jupyter Foundation has already received 1 million USD in income in the year, the funds moved in from LF Charities are charged the lower 6% overhead rate.
...
If keeping funds under the Executive Council's governance matters more than the lower overhead, the money can instead stay in LF Charities. However, until the yearly income there for Jupyter passes 1 million USD, the overhead rate will be the higher 9% rate.

## Grants on behalf of a subproject

A grant "for Jupyter" can be received two ways:

- Into one of the LF entities above, so the money sits inside Jupyter's own structures. Depending on the grant reporting requirements, LF may charge an additional G&A fee to cover the extra administration costs.
